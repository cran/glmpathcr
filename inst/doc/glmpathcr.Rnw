...
The \code{coef} function returns all estimated coefficients for a \code{glmpath.cr} fitted model, where the model selected is indicated by step number, \code{s}. The \code{nonzero.coef} function returns only those non-zero coefficient estimates for a selected model.   
<<echo=TRUE>>=
